...
/*   Updated: 2024/09/17 23:51:36 by erkoc            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	for_fork(t_main *mini, int flag, int i, int fd[2])
{
	set_signal(CHILD_P);
	close(fd[0]);
	dup2(fd[1], 1);
...
int read_and_exec_redirect(t_main *mini, char **tokenized, int i, int fd_2[2])
{
	...
	clean_unnecessary_2(mini, 0, *tokenized, i);
	return (0);
}
...
void	reset(t_main *mini, int *flag)
{
	*flag = 0;
	mini->ifd = -1;
	mini->ofd = -1;
...
int	if_redirect(t_main *mini, char *tokenized, int i, int fd_2[2])
{
	if (read_and_exec_redirect(mini, &tokenized, i, fd_2) == 1)
	{
		set_zero(mini);
		freeall(mini, tokenized);
		return (1);
	}
	return (0);
}
void	if_not_last(t_main *mini, int flag, int i, int fd[2])
{
	if (pipe(fd) == -1)
	{
		perror("Pipe Error\n");
